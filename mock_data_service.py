...
class MegaEnhancedPJMDataService:
    ...
    def get_zone_predictions(self, zone: str, hours_ahead: int = 24) -> Dict:
        ...
        for hour in range(hours_ahead):
            ...
            predictions.append({
                ...
                'confidence_interval': {
                    'lower': round(predicted_price - uncertainty, 2),
    ...
    def get_all_zones_current_status(self) -> Dict:
        ...
        return {
            ...
            'summary': {
                'avg_lmp': round(np.mean([z['current_lmp'] for z in zones_status.values()]), 2),
                'max_lmp': round(max([z['current_lmp'] for z in zones_status.values()]), 2),
                'min_lmp': round(min([z['current_lmp'] for z in zones_status.values()]), 2),
                'total_load': round(sum([z['current_load'] for z in zones_status.values()]), 1),
                'avg_utilization': round(np.mean([z['utilization_pct'] for z in zones_status.values()]), 1)
            },
            'last_updated': current_time.isoformat()
        }
    ...
    def get_zone_wise_lmp(self):
        status_data = self.get_all_zones_current_status()
        return {
            'status': 'success',
            'data': {zone: {'lmp': data['current_lmp'], 'congestion': data['current_congestion'], 
                           'energy': data['current_lmp'] * 0.9, 'loss': random.uniform(0.5, 3)}
